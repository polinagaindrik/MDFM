...
if __name__ == "__main__":
    ...
    add_name_calibr = '_calibr'#'_exps1_9' #'_true' #'_calibr'#
    ...
    for df, predict_name, path_add in zip(dfs_prediction, predict_names, pathes):
        ...
        for _ in range (len(exps_predict_full)):
            x0_bnds_all+=x0_bnds
        prediction_setup['param_bnds'] = x0_bnds_all
        prediction_setup['exps'] = exps_predict_full
        prediction_setup['add_name'] = f'{predict_name}_x0optim'
        prediction_setup['dfs'] = df
        
        start = time.time()
        x0_opt = fm.pest.calculate_prediction(fm.pest.cost_initvals, prediction_setup)[0]
        print((time.time()-start)/60., 'min')
        fm.output.save_values_each_experiment(x0_opt, exps_predict_full, n_cl, dir=path_predict+path_add, filename=f'Initial_values{prediction_setup["add_name"]}')
        x0_opt = fm.output.read_from_json(''+f'Initial_values{prediction_setup["add_name"]}.json', dir=path_predict+path_add)

        #optim_file = f"optimization_history_predict.csv"
        #df_optim = pd.read_csv('out/'s+optim_file)
        #plot_cost_function(df_optim, path=path_predict, add_name=prediction_setup["add_name"])
        
        '''
        x0_opt = df_optim.T[df_optim.T.columns[-1]].values[1:-1]
        save_values_each_experiment(x0_opt, prediction_setup['exps'], n_cl, dir=path_predict+path_add, filename=f'Initial_values{prediction_setup["add_name"]}')
        x0_opt = read_from_json(''+f'Initial_values{prediction_setup["add_name"]}.json', dir=path_predict+path_add)
        '''

        fm.plotting.plot_prediction_result(prediction_setup['param_ode'], x0_opt,  prediction_setup, np.linspace(0, 17, 100),
                            path=path_predict+path_add,
                            clrs=clrs1, add_name=prediction_setup["add_name"])