...
def model_wotemp(n_cl, temps, ntr, s_x, T_x, media, param_ode=None, x10=None, path='', inhib=False, noise=0., rel_noise=0., add_name=''):
    ...
    df_mibi, df_maldi, df_ngs, df_realx, _ = fm.data.generate_data_dfs(fm.mdl.fusion_model_distr, t, param_model, x0, temps, n_cl, n_traj=ntr,
    ...
    fm.data.json_dump({'param_ode': [x00  for i in range (len(temps)) for x00 in x10[i]]+list(param_ode), 's_x': s_x, 'T_x': T_x}, f'Result_real{add_name}.json', dir=path)
