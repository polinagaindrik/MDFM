"""
Generic profile-likelihood engine.

Ported (and generalized) from ``pool_paper_casestudy/profile_likelihood.py``.
That file implemented a fix-one-parameter / re-optimize-the-rest profile
likelihood sweep -- with multiprocessing across grid points and chi2-based
confidence-interval plotting -- but hardcoded every call straight to the
`cost(param, calibr_setup, jac_spasity)` function imported from
`pool_paper_casestudy/do_local_optim.py`. None of the actual algorithm
(grid construction, timeouts, multiprocessing, confidence intervals,
plotting) depends on that specific cost function; it only needs *some*
callable with the `cost_func(param, calibr_setup, jac_spasity)` signature
used throughout this codebase (the same convention as
`fusion_core.pest.calculate_model_params`).

This module is that same engine with `cost_func` threaded through as an
explicit argument instead of a hardcoded import, so it can be reused by any
case study, not just this one. It plays the same role here as
...
as-is for this case study either).

CAVEAT ON THE CHI2 THRESHOLD
-----------------------------
The aggregation function that reduces per-point residuals to a scalar cost
(e.g. `fusion_core.pest.cost_arithmetic_mean`) returns a *mean squared
residual*, not `-2*logL`. The chi2-based confidence interval below is only
statistically exact if `cost_func` is proportional to `-2*logL` (e.g.
`n * MSE / sigma^2` under i.i.d. Gaussian noise). Treat
`confidence_interval_from_profile` as a convenience utility -- calibrate
`scale` (or pass your own threshold) if you need a rigorous interval.
...
levels (n_jobs * per_point_workers <= number of cores). `method="local"`
(L-BFGS-B) is single-threaded per point, so `n_jobs > 1` is the recommended
...
def _cost_fixed_param(free_params, cost_func, fixed_index, fixed_value, calibr_setup, jac_spasity, solve_timeout=30):
    """cost_func() with parameter `fixed_index` clamped to `fixed_value`, rest free.
    ...
    try:
        with time_limit(solve_timeout):
            c = cost_func(full_params, calibr_setup, jac_spasity)
...
def _optimize_one_point(
    cost_func, param_index, val, param_opt, calibr_setup, jac_spasity, method, per_point_workers,
    ...
    if method == "local":
        ...
        for r in range(max(1, n_restarts)):
            ...
            res = minimize(
                ...
                x0_r,
                tol=1e-8,
                args=(cost_func, param_index, val, calibr_setup, jac_spasity),
                method="L-BFGS-B",
                bounds=free_bnds,
        ...
        res = _pest.optimization_func(
            _cost_fixed_param,
            free_bnds,
            args=(cost_func, param_index, val, setup_local, jac_spasity),
...
# calibr_setup / param_opt / cost_func aren't re-pickled for every single task.
_G_COST_FUNC = None
_G_PARAM_OPT = None
...
def _pool_init(cost_func, param_opt, calibr_setup, jac_spasity, method, per_point_workers, n_restarts=1, jitter_frac=0.05):
    global _G_COST_FUNC, _G_PARAM_OPT, _G_CALIBR_SETUP, _G_JAC, _G_METHOD, _G_PER_POINT_WORKERS
    global _G_N_RESTARTS, _G_JITTER_FRAC, _G_RNG
    _G_COST_FUNC = cost_func
...
def _pool_task(task):
    ...
    best_cost, full_p = _optimize_one_point(
        _G_COST_FUNC, param_index, val, _G_PARAM_OPT, _G_CALIBR_SETUP, _G_JAC, _G_METHOD, _G_PER_POINT_WORKERS,
...
def _pool_task_with_timeout(task, timeout=60):
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=1) as ex:
        future = ex.submit(
            _optimize_one_point, _G_COST_FUNC, param_index, val, _G_PARAM_OPT, _G_CALIBR_SETUP,
...
def profile_likelihood_for_param(
    ...
    method             : 'local'  -> scipy.optimize.minimize(L-BFGS-B) from the current best
                          'global' -> fusion_core.pest.optimization_func (differential_evolution),
    ...
    if n_jobs and n_jobs > 1:
        with mp.Pool(
            ...
            initargs=(cost_func, param_opt, calibr_setup, jac_spasity, method, per_point_workers, n_restarts, jitter_frac),
        ) as pool:
...
def confidence_interval_from_profile(grid, profile_cost, cost_opt, confidence_level=0.95, dof=1, scale=1.0):
    ...
    See the CAVEAT at the top of this file: this is exact only if `cost_func`
    is proportional to -2*logL. `scale` lets you calibrate that
    proportionality (e.g. scale = sigma_hat**2 / n if you've estimated a
    residual variance).
    """
    threshold = chi2.ppf(confidence_level, dof) * scale
...
def count_data_points(cost_func, param, calibr_setup, jac_spasity=None):
    """
    Count the number of individual residual terms feeding into cost_func()'s
    real-data residual block, by temporarily swapping in a counting
    aggregation_func instead of `calibr_setup['aggregation_func']` (e.g.
    `fusion_core.pest.cost_arithmetic_mean`). Assumes, like
    `cost_arithmetic_mean`, that the aggregation function receives a list
    whose first entry is the real-data residual array (any further entries,
    e.g. a regularization term, are ignored for the count).
    """
    ...
    def _counting_aggregation(J_vect):
        ...
        # still return a real number so cost_func() doesn't error downstream
        return np.nanmean([np.nanmean(Ji) for Ji in J_vect])

    counting_setup["aggregation_func"] = _counting_aggregation
    cost_func(param, counting_setup, jac_spasity)  # runs cost_func() only to trigger the count
    return counts["n_data"]


def estimate_profile_scale(cost_func, param_opt, calibr_setup, cost_opt, n_free_params, jac_spasity=None):
    """
    Estimate the scale factor converting cost_func() (mean squared residual)
    into an approximate -2*logL scale for the chi2 threshold, assuming i.i.d.
    ...
    (model over-parameterized relative to the data actually used in cost_func()).
    """
...
def run_profile_likelihood_all(
    ...
    """
    param_opt = np.asarray(param_opt, dtype=float)
    cost_opt = cost_func(param_opt, calibr_setup, jac_spasity)
    free_idx = free_param_indices(calibr_setup["param_bnds"])

    if scale == "auto":
        scale, n_data, sigma_hat2 = estimate_profile_scale(
            cost_func, param_opt, calibr_setup, cost_opt, n_free_params=len(free_idx), jac_spasity=jac_spasity
        )
    ...
    if n_jobs and n_jobs > 1:
        ...
        for idx, val in all_tasks:
            best_cost, full_p = _optimize_one_point(
                cost_func, idx, val, param_opt, calibr_setup, jac_spasity, method, per_point_workers,
                n_restarts=n_restarts, jitter_frac=jitter_frac, rng=rng,
            )
    ...
    print("\nApprox. confidence intervals (see CAVEAT in this module's docstring):")
